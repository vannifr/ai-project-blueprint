...
class TestConfidenceScorerBasic:
    ...
    def test_score_returns_float(self, scorer):
        score = scorer.score(
            query="risk classification", title="Risk Framework", body="Classify risk levels."
        )
    ...
    def test_exact_title_match_scores_higher(self, scorer):
        high = scorer.score(
            query="risk classification", title="Risk Classification", body="Some text."
        )
    ...
    def test_keyword_in_body_raises_score(self, scorer):
        with_kw = scorer.score(
            query="gate review", title="Process", body="The gate review is important."
        )
        without_kw = scorer.score(
            query="gate review", title="Process", body="Nothing relevant here."
        )
    ...
    def test_multiple_keyword_occurrences_score_higher(self, scorer):
        ...
        multiple = scorer.score(
            query="risk", title="Risk", body="Risk, risk, and more risk assessment."
        )
